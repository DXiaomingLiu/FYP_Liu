...
if [ $4 == Binary ]; then 
    SELRANGE=`seq 0 20000 20000` # range and step for the selection coefficient to be estimated in 2*Ne units;
...
do
	for SEL in $SELRANGE
	do
    	java -jar $1 -N $NREF -ms $NCHROMS $NREPL -t $THETA -r $RHO $LEN -Sp $SELPOS -SI $SELTIME 1 $FREQ -SAA $(($SEL*2)) -SAa $SEL -Saa 0 -Smark $DEMO -thread 4 | gzip > $2/msms..$SEL..$SELTIME..$i..txt.gz
	done
done 
